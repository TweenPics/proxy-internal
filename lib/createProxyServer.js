...
const rTweenPics = /^http:\/\/i[1-5]?\.tw(?:een|ic)\.pics(?::80)?$|^https:\/\/i[1-5]?\.tw(?:een|ic)\.pics(?::443)?$/;
...
module.exports = ( config, handlers ) => {
        ...
        "port":
            https
                .createServer( config.credentials, ( request, response ) => {
                    ...
                    const urlToLog = `https://${ headers.host }${ url }`;
                    ...
                    if ( redirector ) {
                        ...
                        const target = handleImage( {
                            "authent": config.authent,
                            "origin": config.origin || `https://${ headers.host }`,
                        }, parsedUrl, headers, response );
                        if ( target ) {
    ...
    instance.addRedirector = ( type, notifier ) => {
        let id;
        do {
            id = `/proxy/${ uuid() }`;
        } while ( redirectors.has( id ) );
        redirectors.set( id, notifier );
        return `https://i.${ type }.pics${ id }`;
    };
